...
const projectsPagePath = path.join(webDir, "app", "projects", "page.tsx");
const globalsPath = path.join(webDir, "app", "globals.css");
const outputDir = path.join(rootDir, "output", "validation", "projects-composition");
...
const port = 4860 + Math.floor(Math.random() * 120);
...
async function attachProjectsProviderMocks(context) {
...
async function captureProjects(page, width, height) {
  const routeDir = path.join(outputDir, "projects");
  ...
  try {
    ...
    await page.goto(`${baseUrl}/projects`, { waitUntil: "networkidle", timeout: 45000 });
    await page.waitForSelector(".projects-page", { timeout: 30000 });
    await page.screenshot({
      path: path.join(routeDir, `projects-${width}.png`),
      fullPage: true,
      animations: "disabled",
    });
    ...
    if (width === 1440) {
      await page.locator(".projects-workspace-grid").scrollIntoViewIfNeeded();
      await page.waitForTimeout(180);
      const workspacePath = path.join(routeDir, "projects-workspace-1440.png");
      await page.screenshot({ path: workspacePath, animations: "disabled" });
      supplementalScreenshots.push(path.relative(rootDir, workspacePath).replaceAll("\\", "/"));
    }
    ...
    const metrics = await page.evaluate(({ viewportWidth }) => {
      ...
      const mainRect = document.querySelector(".projects-workspace-main")?.getBoundingClientRect() || null;
      const railRect = document.querySelector(".projects-workspace-rail")?.getBoundingClientRect() || null;

      return {
        viewportWidth,
        headings: Array.from(document.querySelectorAll(".projects-page h1, .projects-page h2"))
          .map((node) => node.textContent?.trim())
          .filter(Boolean),
        focusState: {
          list: document.querySelector('.projects-list-section')?.getAttribute("data-focus-active"),
          publish: document.querySelector('.projects-publish-section')?.getAttribute("data-focus-active"),
          handoff: document.querySelector('.projects-handoff-section')?.getAttribute("data-focus-active"),
        },
        layout: {
        ...
        groups: {
          workspace: summarize(".projects-workspace-grid > *"),
          listEntries: summarize(".projects-list-stack > .dashboard-project-link"),
          publishStates: summarize(".projects-publish-grid > .proof-value-card"),
          railPanels: summarize(".projects-handoff-stack > *"),
        },
      };
    }, { viewportWidth: width });
    ...
    return {
      ...
      overlapWarnings,
      metrics,
      screenshot: path.relative(rootDir, path.join(routeDir, `projects-${width}.png`)).replaceAll("\\", "/"),
...
const sourceChecks = {
  projectsCanvas: fs.readFileSync(projectsPagePath, "utf8").includes("projects-page-canvas"),
  projectsWorkspaceGrid: fs.readFileSync(projectsPagePath, "utf8").includes("projects-workspace-grid"),
  projectsWorkspaceRail: fs.readFileSync(projectsPagePath, "utf8").includes("projects-workspace-rail"),
  projectsPublishStack: fs.readFileSync(projectsPagePath, "utf8").includes("projects-publish-stack"),
  projectsLeadEntry: fs.readFileSync(projectsPagePath, "utf8").includes("projects-list-entry-lead"),
  projectsCanvasCss: fs.readFileSync(globalsPath, "utf8").includes(".projects-page-canvas"),
  projectsRailCss: fs.readFileSync(globalsPath, "utf8").includes(".projects-workspace-rail"),
};

const webStdout = createLogBuffer("[web] ");
const webStderr = createLogBuffer("[web] ");
...
try {
  ...
  await attachProjectsProviderMocks(context);
  await context.addInitScript((modeKey, sessionKey) => {
  ...
  }, E2E_AUTH_MODE_KEY, E2E_AUTH_SESSION_KEY);
  ...
  for (const viewport of [
    ...
    captures.push(await captureProjects(page, viewport.width, viewport.height));
  }
  ...
  fs.writeFileSync(
    path.join(outputDir, "projects-composition-report.json"),
    JSON.stringify(report, null, 2)
  );

  if (!report.passed) {
    throw new Error("projects_composition_validation_failed");
  }

  console.log(
    `Projects composition validation OK. Report: ${path.join(outputDir, "projects-composition-report.json")}`
  );
} catch (error) {
  const errorReport = {
    error: String(error),
  ...
  fs.writeFileSync(
    path.join(outputDir, "projects-composition-error.json"),
