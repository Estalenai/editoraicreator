...
const outputDir = path.join(rootDir, "output", "validation", "anti-block-foundation");
...
const port = 4480 + Math.floor(Math.random() * 120);
...
const sourceChecks = {
  antiBlockMarker: css.includes("Shared anti-block foundation"),
  continuitySeams: css.includes(".dashboard-page > * + *::before") && css.includes(".credits-page-canvas > * + *::before"),
  regionFoundation: css.includes("--ea-flow-region-surface") && css.includes(".projects-page .projects-list-section"),
  itemFoundation: css.includes("--ea-flow-item-surface") && css.includes(".admin-page .admin-subpanel"),
};
...
try {
  ...
  fs.writeFileSync(path.join(outputDir, "anti-block-foundation-report.json"), JSON.stringify(report, null, 2));

  if (!report.passed) {
    throw new Error("anti_block_foundation_validation_failed");
  }

  console.log(`Anti-block foundation validation OK. Report: ${path.join(outputDir, "anti-block-foundation-report.json")}`);
} catch (error) {
  const errorReport = {
    error: String(error),
  ...
  fs.writeFileSync(path.join(outputDir, "anti-block-foundation-error.json"), JSON.stringify(errorReport, null, 2));
  console.error(error);
  process.exitCode = 1;
} finally {
